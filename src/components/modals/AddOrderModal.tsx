...
import { findTableByDeviceId } from "@/lib/mapDeviceToTable";
import {
  addOrderItem,
  decreaseOrderItem,
  deleteOrderItem,
  ...
  const handleViewBill = async () => {
    ...
    await Promise.all([
      ...
      queryClient.refetchQueries({
        queryKey: ["session-orders", table.sessionId],
      }),
      table.sessionId
        ? queryClient.refetchQueries({
    ...
    const devices = queryClient.getQueryData<DevicesResponse>(["devices"]);
    const freshTable = findTableByDeviceId(devices, table.deviceId);

    close();
    openCloseModal(freshTable ?? table);
  };
